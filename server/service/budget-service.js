...
class BudgetService {
  ...
  async getBalance(userId) {
    ...
    const expensesToday = await this.getExpensesPerPeriod(userId, PerDate.perDay);
    const expensesToWeek = await this.getExpensesPerPeriod(
        userId, PerDate.perWeek
    );
    ...
    const incomesToWeek = await this.getIncomesPerPeriod(userId, PerDate.perWeek);
    const incomesToMonth = await this.getIncomesPerPeriod(
        userId,
        PerDate.perMonth
    );
    const incomesToYear = await this.getIncomesPerPeriod(userId, PerDate.perYear);
